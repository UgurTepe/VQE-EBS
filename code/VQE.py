...
max_sample = 10**4  # Max number of samples for EBS
bound = 2*np.pi  # Interval of plot
break_flag = False
max_inside_minima = 4  # Variable to test convergence of algorithm within x% of minima
'''
...
for i in range(max_n):
    ...
    if i > 1 and np.abs(energy + 2) <= 0.2:
        if break_flag == True and it_inside_minima >= max_inside_minima:
            ...
            print(f'At i = {i-max_inside_minima}')
            print(
                '******************************************************************')
            print(f'Energy = {energy}')
            N_step = i - max_inside_minima
            break
        else:
            it_inside_minima += 1
            break_flag = True
...
plt.title(rf'$\epsilon={eps}$, $\alpha ={alpha}$, Number of Steps$={N_step}$')
plt.show()
# np.savetxt(f'data_a{eps:.3f}_e{alpha:.3f}_N{N_step}'+now+'.txt', (arr_par1, arr_par2,arr_energy, arr_var, arr_est_energy, arr_est_var, arr_steps), delimiter=',')
