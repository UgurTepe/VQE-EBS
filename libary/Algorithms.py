...
class eba_geo():
    ...
    def add_sample(self, sample):
        ...
        # Update geometric sampling
        if self.current_step > np.ceil(np.power(self.beta,self.current_step)):
            self.current_k += 1
            self.alpha = np.ceil(np.power(self.beta,self.current_step))/np.ceil(np.power(self.beta,self.current_step+1))
            self.xct = self.alpha*np.log(3/(self.c/self.current_step**self.p))
    ...
    # Just a function to calculate c_t for a given time t
    def calc_ct(self, time):
        return np.sqrt(2*self.running_variance[-1]*self.xct/time)+(3*self.range_of_rndvar*self.xct)/time
